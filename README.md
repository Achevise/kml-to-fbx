...
- `--polygon-outline-width auto|FLOAT|PERCENT%`: modo de tamaño del contorno de anillos de `Polygon`.
  - Default: `auto`.
  - `auto`: calcula el grosor como el `5%` del tamaño del bounding box mas grande de toda la jerarquia procesada.
  - `FLOAT` (ej. `1`): grosor fijo en metros.
  - `PERCENT%` (ej. `5%`): porcentaje del tamaño del bounding box mas grande de toda la jerarquia procesada.
  ...
  - En modos `auto` y `%`, todos los outlines del archivo usan exactamente el mismo grosor global.
