...
def _shape_to_mesh(shape, point_radius: float, line_width: float, polygon_height: float):
    if shape.geometry_type == "Point":
...
def main(argv: List[str] | None = None) -> int:
    ...
    for item in projected:
        ...
        if item["geometry_type"] == "Polygon" and args.polygon_outline_width > 0.0:
            outline = polygon_outline_mesh(item["coordinates"], args.polygon_outline_width)
            if args.flip_winding:
